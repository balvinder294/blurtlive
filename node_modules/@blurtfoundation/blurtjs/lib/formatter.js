"use strict";

var _slicedToArray = function () { function sliceIterator(arr, i) { var _arr = []; var _n = true; var _d = false; var _e = undefined; try { for (var _i = arr[Symbol.iterator](), _s; !(_n = (_s = _i.next()).done); _n = true) { _arr.push(_s.value); if (i && _arr.length === i) break; } } catch (err) { _d = true; _e = err; } finally { try { if (!_n && _i["return"]) _i["return"](); } finally { if (_d) throw _e; } } return _arr; } return function (arr, i) { if (Array.isArray(arr)) { return arr; } else if (Symbol.iterator in Object(arr)) { return sliceIterator(arr, i); } else { throw new TypeError("Invalid attempt to destructure non-iterable instance"); } }; }(); // import get from "lodash/get";


var _ecc = require("./auth/ecc");

module.exports = function (blurtAPI) {
    function numberWithCommas(x) {
        return x.replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    }

    function vestingBlurt(account, gprops) {
        var vests = parseFloat(account.vesting_shares.split(" ")[0]);
        var total_vests = parseFloat(gprops.total_vesting_shares.split(" ")[0]);
        var total_vest_blurt = parseFloat(gprops.total_vesting_fund_blurt.split(" ")[0]);
        var vesting_blurtf = total_vest_blurt * (vests / total_vests);
        return vesting_blurtf;
    }

    function calculateSaving(savings_withdraws) {
        var savings_pending = 0;
        savings_withdraws.forEach(function (withdraw) {
            var _withdraw$amount$spli = withdraw.amount.split(" "),
                _withdraw$amount$spli2 = _slicedToArray(_withdraw$amount$spli, 2),
                amount = _withdraw$amount$spli2[0],
                asset = _withdraw$amount$spli2[1];

            if (asset === "BLURT") savings_pending += parseFloat(amount);
        });
        return { savings_pending: savings_pending };
    }

    function estimateAccountValue(account) {
        var _ref = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {},
            gprops = _ref.gprops,
            savings_withdraws = _ref.savings_withdraws,
            vesting_blurt = _ref.vesting_blurt;

        var promises = [];
        var username = account.name;
        var assetPrecision = 1000;
        var savings = void 0;

        if (!vesting_blurt) {
            if (!gprops) {
                promises.push(blurtAPI.getStateAsync("/@" + username).then(function (data) {
                    gprops = data.props;
                    vesting_blurt = vestingBlurt(account, gprops);
                }));
            } else {
                vesting_blurt = vestingBlurt(account, gprops);
            }
        }

        if (!savings_withdraws) {
            promises.push(blurtAPI.getSavingsWithdrawFromAsync(username).then(function (savings_withdraws) {
                savings = calculateSaving(savings_withdraws);
            }));
        } else {
            savings = calculateSaving(savings_withdraws);
        }

        return Promise.all(promises).then(function () {
            var savings_balance = account.savings_balance;
            var balance_blurt = parseFloat(account.balance.split(" ")[0]);
            var saving_balance_blurt = parseFloat(savings_balance.split(" ")[0]);

            var total_blurt = vesting_blurt + balance_blurt + saving_balance_blurt + savings.savings_pending;

            return total_blurt.toFixed(2);
        });
    }

    function createSuggestedPassword() {
        var PASSWORD_LENGTH = 32;
        var privateKey = _ecc.key_utils.get_random_key();
        return privateKey.toWif().substring(3, 3 + PASSWORD_LENGTH);
    }

    return {
        reputation: function reputation(_reputation) {
            if (_reputation == null) return _reputation;
            _reputation = parseInt(_reputation);
            var rep = String(_reputation);
            var neg = rep.charAt(0) === "-";
            rep = neg ? rep.substring(1) : rep;
            var str = rep;
            var leadingDigits = parseInt(str.substring(0, 4));
            var log = Math.log(leadingDigits) / Math.log(10);
            var n = str.length - 1;
            var out = n + (log - parseInt(log));
            if (isNaN(out)) out = 0;
            out = Math.max(out - 9, 0);
            out = (neg ? -1 : 1) * out;
            out = out * 9 + 25;
            out = parseInt(out);
            return out;
        },

        vestToBlurt: function vestToBlurt(vestingShares, totalVestingShares, totalVestingFundBlurt) {
            return parseFloat(totalVestingFundBlurt) * (parseFloat(vestingShares) / parseFloat(totalVestingShares));
        },

        commentPermlink: function commentPermlink(parentAuthor, parentPermlink) {
            var timeStr = new Date().toISOString().replace(/[^a-zA-Z0-9]+/g, "").toLowerCase();
            parentPermlink = parentPermlink.replace(/(-\d{8}t\d{9}z)/g, "");
            return ("re-" + parentAuthor + "-" + parentPermlink + "-" + timeStr).replace(/\./g, "");
        },

        amount: function amount(_amount, asset) {
            return _amount.toFixed(3) + " " + asset;
        },
        numberWithCommas: numberWithCommas,
        vestingBlurt: vestingBlurt,
        estimateAccountValue: estimateAccountValue,
        createSuggestedPassword: createSuggestedPassword
    };
};