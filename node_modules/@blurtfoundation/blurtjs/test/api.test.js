...
describe("blurt.api:", function () {
    this.timeout(30 * 1000);

    describe("setOptions", () => {
        it("works", () => {
            let url = blurt.config.get("uri");
            if (!url) url = blurt.config.get("websocket");
            blurt.api.setOptions({ url: url, useAppbaseApi: true });
        });
    });
    ...
    describe("getFollowers", () => {
        describe("getting ned's followers", () => {
            it("works", async () => {
                const result = await blurt.api.getFollowersAsync(
                    "ned",
                    0,
                    "blog",
                    5
            ...
            it("the startFollower parameter has an impact on the result", async () => {
                // Get the first 5
                const result1 = await blurt.api.getFollowersAsync(
                    "ned",
                    0,
                    "blog",
                    5
                ...
                const result2 = await blurt.api.getFollowersAsync(
                    "ned",
            ...
            it("clears listeners", async () => {
                blurt.api.listeners("message").should.have.lengthOf(0);
            });
        });
    });
    ...
    describe("getContent", () => {
        describe("getting a random post", () => {
            it("works", async () => {
                const result = await blurt.api.getContentAsync(
                    "yamadapc",
    ...
    describe("streamBlockNumber", () => {
        it("streams blurt transactions", (done) => {
            let i = 0;
            const release = blurt.api.streamBlockNumber((err, block) => {
                should.exist(block);
    ...
    describe("streamBlock", () => {
        it("streams blurt blocks", (done) => {
            let i = 0;
            const release = blurt.api.streamBlock((err, block) => {
                try {
                    should.exist(block);
    ...
    describe("streamTransactions", () => {
        it("streams blurt transactions", (done) => {
            let i = 0;
            const release = blurt.api.streamTransactions((err, transaction) => {
                try {
    ...
    describe("streamOperations", () => {
        it("streams blurt operations", (done) => {
            let i = 0;
            const release = blurt.api.streamOperations((err, operation) => {
                try {
    ...
    describe("with retry", () => {
        ...
        it("works by default", async function () {
            ...
            const result = await blurtApi.getFollowersAsync(
                "ned",
                0,
                "blog",
                5
        ...
        it("does not retry by default", async () => {
            ...
            let errored = false;
            try {
                result = await blurtApi.getFollowersAsync("ned", 0, "blog", 5);
            } catch (e) {
                errored = true;
            }
            assert.equal(attempts, 1);
