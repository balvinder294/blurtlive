...
if (_cluster["default"].isMaster) console.log('application server starting, please wait.'); // import uploadImage from 'server/upload-image' //medium-editor

var app = new _koa["default"]();
app.name = 'Blurt app';
var env = process.env.NODE_ENV || 'development'; // cache of a thousand days
...
var adstxt = _fs["default"].readFileSync(_path["default"].join(__dirname, '../app/assets/ads.txt'), 'utf8'); // Serve static assets without fanfare
...
app.use((0, _koaIsbot["default"])()); // set number of processes equal to number of cores
...
if (env !== 'test') {
  var appRender = require('./app_render'); // Load special posts and store them on the ctx for later use. Since
  // we're inside a generator, we can't `await` here, so we pass a promise
  // so `src/server/app_render.jsx` can `await` on it.


  app.specialPostsPromise = (0, _SpecialPosts.specialPosts)(); // refresh special posts every five minutes
