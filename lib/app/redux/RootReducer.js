...
function initReducer(reducer, type) {
  return function (state, action) {
    if (!state) return reducer(state, action); // @@redux/INIT server and client init
    ...
    if (action.type === '@@router/LOCATION_CHANGE' && type === 'global') {
      state = state.set('pathname', action.payload.pathname); // console.log(action.type, type, action, state.toJS())
    }
