...
var linksRe = _interopRequireWildcard(require("app/utils/Links"));
...
describe('Links', function () {
  ...
  it('by domain', function () {
    ...
    matchNot(linksRe.local(), remotes); // match(linksRe({external: false}), largeData + 'https://blurt.world2/next', 'https://blurt.world2/next')
  });
...
var compare = function compare(matching, re, input) {
  ...
  if (matching) {
    (0, _assert["default"])(m, "No match --> ".concat(input, " --> output ").concat(output, " --> using ").concat(re.toString())); // console.log('m', m)
