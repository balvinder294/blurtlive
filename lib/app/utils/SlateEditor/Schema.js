...
var HtmlRules = [// Catch-all debug wrapper
...
}, // Block rules
{
  deserialize: function deserialize(el, next) {
    ...
    if (!type) return; // Special case for <pre>: ignore its inner <code> element.

    var code = el.tagName == 'pre' ? el.children[0] : null;
    var children = code && code.tagName == 'code' ? code.children : el.children; // due to disabled/broken whitespace normalization in cheerio/htmlparser2, perform basic cleaning...
    ...
    children = children.reduce(function (out, child) {
      if (child.tagName == 'center') {
        center = true; // child.children.map(c => out.push(c))
...
var getMarkdownType = function getMarkdownType(chars) {
  switch (chars) {
    case '1.':
    case '*':
    case '-':
...
exports.getMarkdownType = getMarkdownType;