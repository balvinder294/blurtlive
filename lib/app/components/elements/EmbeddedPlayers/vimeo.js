...
function extractMetadata(data) {
  ...
  return {
    ...
    canonical: "https://player.vimeo.com/video/".concat(m[1]) // thumbnail: requires a callback - http://stackoverflow.com/questions/1361149/get-img-thumbnails-from-vimeo

  };
}

function embedNode(child, links
/* images */
) {
  try {
    var data = child.data;
    ...
    if (links) links.add(vimeo.canonical); // if(images) images.add(vimeo.thumbnail) // not available
  } catch (error) {
    console.log(error);
  }
